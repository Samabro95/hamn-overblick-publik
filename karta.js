...
// Luft runt fartygen och hamnarna i startläget, i landdatans pixlar: 3600 px
// täcker 38 longitudgrader, så 150 px är omkring 1,6° eller nio mil vid 58°N.
// Marginalen sätter också hur långt ut man får zooma, eftersom klamra() inte
// släpper förbi startläget. Låg först på 70 — knappt fyra mil — vilket gav en
// bild som satt tätt inpå trafiken utan överblick över farvattnen omkring.
const MARGINAL = 150;
const MAX_ZOOM = 20;
...
function skapaKarta(svg, land, utseende = {}) {
  ...
  function ritaHamnar(ports, { antal = null, historikläge = false } = {}) {
    const lager = svg.querySelector('#hamnrutor');
    lager.textContent = '';
    for (const p of ports) {
      const [[laMin, loMin], [laMax, loMax]] = p.box;
      const ruta = el('rect', {
        class: 'hamnruta' + (historikläge ? ' klickbar' : ''),
      ...
      lager.appendChild(ruta);

      if (historikläge) {
        // Kanten är streckad och tunn; utan en fylld yta finns inget att
        // träffa med pekaren annat än själva strecket.
        ruta.setAttribute('fill', HAMNFARG(p.name));
        ruta.setAttribute('fill-opacity', '0.08');
        ruta.addEventListener('click', (ev) => {
          ev.stopPropagation();
          hamnKlickLyssnare(p.name, skärmläge(px(loMax), py(laMax)));
        });
      }

      const n = antal?.get(p.name);
      if (n) {
        // En siffra ovanpå kartan försvinner mot land och hav. Brickan ger den
        // en egen bakgrund, med hamnens färg som kant så att den hör ihop med
        // rutan och med teckenförklaringen i hamnfiltret.
        const bredd = s_(13 + String(n).length * 7);
        const höjd = s_(16);
        const bx = px(loMax) - bredd - s_(2);
        const by = py(laMax) + s_(2);

        const bricka = el('rect', {
          class: 'hamnbricka', x: bx, y: by, width: bredd, height: höjd,
          rx: s_(3), stroke: HAMNFARG(p.name), 'stroke-width': s_(1.2),
        });
        const siffra = el('text', {
          class: 'hamnantal', fill: HAMNFARG(p.name),
          x: bx + bredd / 2, y: by + höjd - s_(4.5),
          'text-anchor': 'middle', 'font-size': s_(11),
        });
        siffra.textContent = n;
        lager.append(bricka, siffra);

        if (historikläge) {
          // Brickan är det man siktar på när rutan är liten.
          for (const träffyta of [bricka, siffra]) {
            träffyta.style.cursor = 'pointer';
            träffyta.addEventListener('click', (ev) => {
              ev.stopPropagation();
              hamnKlickLyssnare(p.name, skärmläge(px(loMax), py(laMax)));
            });
          }
  ...
  function ritaFartyg(fartyg, valdMmsi, { historikläge = false } = {}) {
    ...
    // I historikfliken är fartygen bakgrund: det är hamnarnas siffror man läser.
    lager.classList.toggle('historik', historikläge);
  ...
  return {
    ...
    påHamnKlick: (fn) => { hamnKlickLyssnare = fn; },
